...
(** register schemas *)
let register_node_schema_capnp = Props_capnp.register_node_schema
let register_edge_schema_capnp = Props_capnp.register_edge_schema

(** set/get node properties with capnproto *)
let set_node_props_capnp = Props_capnp.set_node_props_capnp
let get_node_props_capnp = Props_capnp.get_node_props_capnp

(** set/get edge properties with capnproto *)
let set_edge_props_capnp = Props_capnp.set_edge_props_capnp
let get_edge_props_capnp = Props_capnp.get_edge_props_capnp
...
let get_node_info (db : t) (node_id : node_id) : node_info option =
  try
    let key = Keys.encode_id node_id in
    let blob = Lmdb.Map.get db.nodes key in
    ...
    let node_type = Store.unintern db type_id in
...
let create_node (db : t) (node_type : string) : node_id =
  ...
  Lmdb.Map.set db.nodes (Keys.encode_id node_id) blob;
  node_id
...
(** delete a node *)
let delete_node (db : t) (node_id : node_id) : unit =
  try
    Lmdb.Map.remove db.nodes (Keys.encode_id node_id)
...
let delete_edge (db : t) (edge_id : edge_id) : unit =
  try
    ...
    Lmdb.Map.remove db.edges (Keys.encode_id edge_id);
    ...
    Lmdb.Map.remove db.outbound outbound_key;
    Lmdb.Map.remove db.inbound inbound_key
